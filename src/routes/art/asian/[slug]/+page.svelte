<script lang="ts">
	...
	async function getNumOfDbRows(): Promise<number | undefined> {
		...
		const { data: asianData, error: asianDataError } = await supabase
			.from(dbArtworksName)
			.select(
				`
			id
	...
	async function getArtworks(): Promise<ArtWork[]> {
		...
		*/
		...
		if (asianArtworksData != null && asianArtworksData.length >= 0) {
			...
			console.log('getartworks error' + asianArtworksError);
			return [];
		}
	}
